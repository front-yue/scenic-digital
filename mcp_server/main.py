...
def get_map_route(start_point: str, end_point: str) -> str:
    ...
    Args:
        start_point: 起点名称 (例如：景区南门、云海观景台)。如果用户说“从这里”、“当前位置”、“我这里”、“这儿”等，必须传入默认值“浙大科技园湖州基地”。
    ...
    """
    # 0. 智能处理“当前位置”
    # 使用模糊匹配，只要大模型传进来的词包含以下任何一个关键字，就强制替换
    if any(keyword in start_point for keyword in ["这里", "当前", "这儿", "我", "西站", "游客中心", "景区南门"]):
        start_point = "浙大科技园湖州基地" 
    ...
    if not origin_coord or not dest_coord:
        return f"抱歉，我无法在地图上准确定位【{start_point}】或【{end_point}】，请提供更具体的地点名称。"
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            ...
            if route_data.get('status') == '1' and route_data.get('route') and route_data['route'].get('paths'):
                ...
                result = f"为您规划了从【{start_point}】到【{end_point}】的路线：全程大约 {distance} 米，预计需要 {duration} 分钟。主要路线：{instruction_text}。"
                return result
            else:
                # 增加对驾车/公交的兼容提示
                return f"抱歉，【{start_point}】到【{end_point}】距离可能过远，超出了步行导航的范围，建议您使用地图 APP 选择驾车或公交出行。"
    except Exception as e:
